    ...
    <>
      ...
      >
        ...
        <div className="mx-auto relative flex flex-col md:flex-row gap-5 md:gap-20 items-start justify-center">
          <img
            className="shadow-slate-950 shadow-lg rounded-lg w-40 md:w-60 lg:w-72 mt-6"
          ...
          <div className="text-white max-w-lg space-y-4">
            ...
            <div className="flex flex-wrap py-3 gap-3">
              {movie.genres && movie.genres.length > 0 ? (
                movie.genres.map((genero) => (
                  <h2
                    className="font-semibold border-2 py-1 px-2 rounded-lg border-red-950 text-sm"
                    key={genero.id}
                  >
                    {genero.name}
                  </h2>
            ...
            <div className="flex flex-col gap-3">
              {/* Botão para Filmes Assistidos */}
              <span className="text-white mt-5">{filmesAssistido.some((filme) => filme.id === movie.id) ? 'Filmes Favoritos:' : 'Adicionar a Filmes Favoritos: '}</span>
              <button
                title="Add to Watched"
                className="group cursor-pointer outline-none mt-5"
              ...
              >
                <svg
                  ...
                  height="50px"
                  viewBox="0 0 24 24"
                  className={`stroke-${filmesAssistido.some((filme) => filme.id === movie.id) ? 'red-800' : 'zinc-400'} fill-none group-hover:stroke-red-800 group-active:stroke-red-950 duration-300`}
                >
                  <path
              ...
              {/* Botão para Filmes Para Assistir */}
              <span className="text-white mt-5">Filmes para Assistir: </span>
              <button
      ...
      {/* Elenco */}
      <div className="mt-10 px-5 md:px-10 w-full md:w-5/6 mx-auto h-auto">
